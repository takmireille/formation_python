...
def create_groups(group_names):
    """
    Crée plusieurs groupes d'utilisateurs
...
def create_users(fichier_csv):
    # lire le fichier user.csv
    # extraire , concatener les nom et prenoms pour former les noms des users
    # creer les users
    # les ajoutés aux groupes d'utlisateurs 
    try:
        with open(fichier_csv, 'r') as file:
            ...
            for row in csv_reader:
                ...
                try:
                    os.system(f'sudo useradd -mG {groupe} ; sudo passwd -e {username}')
...
group_names = ["Comptabilite", "Ressources_Humaines"]
